...
impl Lexer {
    ...
    pub fn analisar(&mut self) {
        ...
        self.lista_tokens.push(Token::novo(
            Tipo_Token::EOF,
            "EOF".into(),
            "EOF".into(),
            self.linha,
    ...
    fn scan_token(&mut self) {
        ...
        match c {
            '(' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::PARENTESE_ESQUERDO)),
            ')' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::PARENTESE_DIREITO)),
            '{' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::CHAVE_ESQUERDA)),
            '}' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::CHAVE_DIREITA)),
            ',' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::VIRGULA)),
            ';' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::PONTO_VIRGULA)),
            '=' => {
                if self.v_prox('=') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_D_IGUAL))
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_IGUAL))
                }
            }
            '|' => {
                if self.v_prox('|') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_D_OR))
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_OR))
                }
            }
            '&' => {
                if self.v_prox('&') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_D_AND));
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_AND));
                }
            }
            '!' => {
                if self.v_prox('=') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_D_DIFERENTE))
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_NOT))
                }
            }
            '<' => {
                if self.v_prox('=') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_MENOR_IGUAL_Q))
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_MENOR_Q))
                }
            }
            '>' => {
                if self.v_prox('=') {
                    self.proximo();
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_MAIOR_IGUAL_Q))
                } else {
                    self.lista_tokens
                        .push(self.montar_token(Tipo_Token::SIMBOLO_MAIOR_Q))
                }
            }
            '+' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_MAIS)),
            '-' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_MENOS)),
            '*' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_MULTI)),
            '/' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_DIV)),
            '%' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_MOD)),
            '~' => self
                .lista_tokens
                .push(self.montar_token(Tipo_Token::SIMBOLO_BIT_NOT)),
            ' ' => {}
            '\n' => self.linha += 1,
            '\r' => {}
            '\t' => {}
    ...
    fn peek(&self) -> char {
        if self.char_atual >= self.fonte.len() {
            return '\0';
        }
    ...
    fn peek_prox(&self) -> char {
        if (self.char_atual + 1) >= self.fonte.len() {
            return '\0';
        }
    ...
    fn montar_token_2(&self, tipo: Tipo_Token, literal: String) -> Token {
        Token::novo(
            tipo,
            String::from_utf8(self.fonte[self.comeco..self.char_atual].to_vec()).unwrap(),
            literal,
            self.linha,
        )
    }
    ...
    fn pegar_string(&mut self) -> Token {
        ...
        if self.char_atual >= self.fonte.len() {
            panic!("string incompleta na linha {}", self.linha);
        }
        self.proximo();
    ...
    fn pegar_numero(&mut self) -> Token {
        ...
        while (self.peek().is_numeric()) && !(self.char_atual >= self.fonte.len()) {
            self.proximo();
        }
    ...
    fn pegar_id(&mut self) -> Token {
        while alpha_numerico_underline(self.peek()) {
            self.proximo();
        }
